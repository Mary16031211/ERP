...
{	static final int MaxNum=20;//图的最大顶点数
    ...
    int[] isTrav=new int[MaxNum]; 				//定义邻接矩阵图矩阵 
}			
...
public class P2_6 {
	...
	static void OutGraph(GraphMatrix GM)			//显示图
	{
	    int i,j;
	    for(j=0;j<GM.VertexNum;j++)
		{
	    ...
	    for(i=0;i<GM.VertexNum;i++) 
	    {
	        ...
	        for(j=0;j<GM.VertexNum;j++)
	        {
	            if(GM.EdgeWeight[i][j]==GraphMatrix.MaxValue) //若权值最大
				{
	                System.out.printf("\tZ");          		//用Z表示无穷大
				}
	            else
				{
	...
	public static void main(String[] args) {
	    ...
	    GM.GType=input.nextInt();			//图的类型
	    System.out.printf("输入图的顶点数量:");
	    GM.VertexNum=input.nextInt();		//图的顶点数
	    System.out.printf("输入图的边的数量:");
	    GM.EdgeNum=input.nextInt();			//图的边数 
		ClearGraph(GM);					//清空图
